"""
    251128 Cleaninig
    TimmViTDecoderODP is removed.
    TimmViTDecoderToMe is removed.
"""
import torch
...
class TimmViTDecoder(nn.Module):
    ...
    def forward(self, z, interpolate_zq=None, H=None, W=None):
        ...
        if hasattr(self, 'latent_pos_embed'):
            n = z.shape[1]
            z = z + self.latent_pos_embed[:, :n, :]
...
# ######################################################################################################################################
# ######################################################################################################################################
# class TimmViTDecoderODP(TimmViTDecoder):
#     def forward(self, z, interpolate_zq=None, H=None, W=None):
#         if H is None:
#             num_img_tokens = self.num_img_tokens
#             H = W = int(math.sqrt(num_img_tokens)) * self.patch_size
