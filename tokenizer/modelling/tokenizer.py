...
class ModelArgs:
    ...
    variable_method: str = 'none'
    merge_dim: int = 768
    vf_loss_weight: float = 1.0
    vf_margin: float = 0.25
    ...
    def encode(self, x):
        ...
        if self.repa and self.training:
            ...
            if self.repa_align == 'global':
                ...
                z_hat = self.projection(quant)  # B x N x D
                b, l, d = z_hat.shape
...
class MergeModule(nn.Module):
    def __init__(self, num_latent_tokens, dim, vf_margin=0.25):
        super().__init__()

        self.num_latent_tokens = num_latent_tokens
        self.dim = dim
        self.vf_margin = vf_margin
        
        # embeddings
        self.embeddings = nn.Parameter(torch.randn(num_latent_tokens, dim), requires_grad=True)
        with torch.no_grad():
            self.embeddings.data = F.normalize(self.embeddings.data, dim=-1)
        
        # mask
        max_len = self.num_latent_tokens
        self.mask = torch.block_diag(torch.ones(max_len // 4, max_len // 4),
    ...
    def forward(self, x, num):
        ...
        if self.training:
            with torch.amp.autocast('cuda', enabled=False):
                embeddings = self.embeddings.float()
                metric = F.normalize(embeddings, dim=-1)
                
                with torch.no_grad():
                    tmp = metric.float()
                    scores = tmp @ tmp.T
                    scores = scores + torch.randn_like(scores).clamp(-1, 1) * 0.1   # for exploration
                    mask = self.mask.to(scores.device)
                    scores = scores * mask + (1 - mask) * -100.
                    ...
                    # prototype
                    tmp = assignment / assignment.sum(dim=0, keepdim=True)
                    prototype = F.normalize(tmp.T @ metric, dim=-1)
                prototype = prototype.detach()
                tau = 0.1
                logits = metric @ prototype.T / tau
                da = assignment + F.softmax(logits, dim=-1) - F.softmax(logits, dim=-1).detach()
                mat = (da / da.sum(dim=0, keepdim=True)) @ da.T
            da = da.to(dtype=x_dtype)
            mat = mat.to(dtype=x_dtype)
            out_x = torch.einsum('bnc,nm->bmc', x, mat)
            
            # additional loss
            a = F.normalize(x, dim=-1)  # B x N x D
            sim = torch.einsum('bnc,bmc->bnm', a, a).mean(dim=0).detach()  # N x N
            sim = sim * self.mask.to(sim.device)
            b = F.normalize(embeddings, dim=-1)
            sim2 = torch.einsum('nc,mc->nm', b, b)  # N x N
            sim2 = sim2 * self.mask.to(sim2.device)
            diff = torch.abs(sim - sim2)
            mask_on = self.mask.to(diff.device)
            vf_loss = (F.relu(diff - self.vf_margin) * mask_on).sum() / mask_on.sum()
            return out_x.to(dtype=x_dtype), da, mat, vf_loss
        ...
        mat = mat.to(dtype=x_dtype)
        out_x = torch.einsum('bnc,nm->bmc', x, mat)
        return out_x.to(dtype=x_dtype), da, mat, None    


class SoftVQModel2(SoftVQModel):
    """
        SoftVQ with DINO decoder auxiliary loss replacing repa loss
    """
    def __init__(self, config: ModelArgs):
        super().__init__(config)
        
        # auxiliary based REPA
        if self.repa:
            # remove projection
        ...
        # variable length module
        self.variable_method = config.variable_method
        if self.variable_method == 'ours2':
            self.merge_module = MergeModule(config.num_latent_tokens, config.merge_dim, vf_margin=config.vf_margin)
        elif self.variable_method != 'none':
            raise ValueError(f"Unsupported variable_method: {self.variable_method}")
    ...
    def encode(self, x, num=None):
        ...
        if self.variable_method == 'ours2':
            if self.training:
                values = torch.tensor([32, 48, 64, 96, 128, 196, 256])
                weights = torch.tensor([2.0, 2.0, 2.0, 1.0, 1.0, 1.0, 1.0])
                idx = torch.multinomial(weights, num_samples=1)
                num = values[idx].item()
            else:
                if num is None:
                    num = 32    # fixed length for inference
            
            if num < self.config.num_latent_tokens:
                quant, _, _, vf_loss = self.merge_module(quant, num=num)
        
        if self.repa and self.training:
            # decode dinov2 feature
            dec_dino = self.decode_dino(quant, x=x, h=height, w=width)
            dec_dino = self.to_pixel_dino(dec_dino)
            dec_dino = F.normalize(dec_dino, dim=-1)
            ...
            emb_loss += (dino_rec_loss,)
            
        if self.variable_method == 'ours2':
            if vf_loss is not None:
                emb_loss += (vf_loss * self.config.vf_loss_weight,)
        return quant, emb_loss, info
    
    def train(self, mode = True):
        super().train(mode)
        if self.repa:
            self.repa_model.eval()  # always eval
        return self

    def forward(self, input, num=None):
        b, _, h, w = input.size()
        quant, diff, info = self.encode(input, num)
        self.quant = quant
        dec = self.decode(quant, x=input, h=h, w=w)
        return dec, diff, info
...
def SoftVQ2(**kwargs):
    return SoftVQModel2(ModelArgs(encoder_ch_mult=[1, 1, 2, 2, 4], decoder_ch_mult=[1, 1, 2, 2, 4], **kwargs))
