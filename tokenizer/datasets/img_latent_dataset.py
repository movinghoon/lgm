...
class ImgLatentDataset(Dataset):
    def __init__(self, data_dir, latent_norm=True, latent_multiplier=1.0, scalar_norm=False):
        ...
        if latent_norm and not scalar_norm:
            self._latent_mean, self._latent_std = self.get_latent_stats()
            
        elif latent_norm and scalar_norm:
            _latent_mean, _latent_std = self.get_latent_stats()
            self._latent_mean = torch.mean(_latent_mean)
            self._latent_std = (_latent_std.pow(2).mean() + _latent_mean.pow(2).mean() - self._latent_mean.pow(2)).sqrt()
